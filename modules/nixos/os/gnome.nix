{
  flake.modules.nixos.custom =
    ...
    let
      ...
      mkNvidiaSuspendFix = import ../../../lib/mkNvidiaSuspendFix.nix { inherit lib pkgs; };
        ...
        lib.mkMerge [
          {
            ...
            environment.systemPackages = lib.mkIf cfg.extensions.enable (
            ...
            environment.gnome.excludePackages = lib.mkIf cfg.strip.enable (
              with pkgs;
              [
                cheese
                epiphany
