# attic — headless server hosting the attic binary cache. STABLE nixpkgs,
# no home-manager, no binary-cache substituter (it *is* the cache), cuda cache
# on. Does not use `common`/`baseline`; sets its own locale/timezone inline.
{ config, inputs, ... }:
let
...
in
{
  flake.nixosConfigurations.attic = inputs.nixpkgs.lib.nixosSystem {
    system = "x86_64-linux";
    ...
    modules = [
      ...
      ../../machines/attic/hardware-configuration.nix
