...
let
  steamSession = pkgs.writeShellScript "steam-gamescope-session" ''
    ...
    # Default: gamescope composites on the Intel iGPU (it drives the panel) and
    # games offload to the GTX 1060 via PRIME. If Big Picture is slow or blank,
    # force the whole session onto NVIDIA the way the desktop steam-os.nix does:
    # WLR_RENDERER=vulkan, GBM_BACKEND=nvidia-drm.
